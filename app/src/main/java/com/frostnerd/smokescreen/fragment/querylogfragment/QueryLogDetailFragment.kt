        ...
        createDnsRule.setOnClickListener {
            ...
            if(query != null) {
                val answerIP = query.parsedResponses.firstOrNull {
                    it.type == query.type
                }?.payload?.toString() ?: if(query.type == Record.TYPE.A) "127.69.69.69" else "::1"
    ...
    private fun updateUi() {
        ...
        if(query != null && viewCreated) {
            ...
            protocol.text = when {
                ...
                query.askedServer!!.startsWith("tls") -> getString(R.string.fragment_querydetail_mode_doh)
                query.askedServer!!.startsWith("quic") -> getString(R.string.fragment_querydetail_mode_doq)
                else -> "-"
            }
